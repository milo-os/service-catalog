...
To make those classes reachable from consumer projects, the team adds a `spec.provisioning` block to the `ServiceConfiguration` it already publishes — the same document where it declares its metrics, its quota limits, and the location classes it runs on. Each entry names the source project, a kind, and a label selector: *the classes in my project carrying this label should be usable from every project that enables me.* It names no consumer project, enumerates no classes, and writes no manifest.
...
| Risk | Mitigation |
| --- | --- |
| The mechanism becomes a general "write anything anywhere" primitive | Platform-owned allowlist of installable kinds, enforced at admission and again in the controller — not left to RBAC, which is not an effective ceiling |
...
**The allowlist.** A platform-owned table names every kind a service may ask the platform to install, and fixes everything about the produced object other than which source objects are selected: the served version, the name, and the spec. It is static configuration in the platform repository, not a field of any `ServiceConfiguration`. It is enforced twice — at admission, so a provider gets a synchronous error rather than discovering the refusal in a consumer's status later, and again in the controller before any write, because the webhook can be absent from the cluster and a configuration admitted under an older, wider allowlist stays in etcd. The second check is not redundant with the first; an e2e suite runs with the webhook disabled to prove the controller refuses on its own.

**A categorical deny list.** RBAC objects, IAM policy bindings, service accounts, secrets, admission webhook configurations, and custom resource definitions are excluded outright rather than by omission, so adding an allowlist entry cannot quietly re-admit one. These are the kinds whose installation converts "a provider can create an object" into "a provider can act as someone else, indefinitely, after the entitlement is gone." That rules out per-project RBAC or IAM bindings, which is the right outcome: the need is real and should be met the way quota was, as a separate typed fan-out where the provider names only its own roles and the platform chooses the subject and the scope.
...
Nothing in this version closes that. The gap is reported instead of hidden: the allowlist records, per kind, whether the target API performs its own check, and each project's entitlement status carries `authorizationEstablished` per resource with a consumer-facing explanation of what it means.
...
| Declaration withdrawn, or its kind removed from the allowlist | Pruned, scoped by service, entitlement, and declaration name, so one service's fan-out cannot remove another's objects |
...
Provisioning runs only for services whose `ServiceConfiguration` declares `spec.provisioning`, and only for kinds in the platform allowlist; removing the section disables it for one service, and emptying the allowlist disables it globally. Neither requires downtime, and re-enabling is idempotent because the content is derived rather than authored.
...
- 2026-08-17 — projection delivery, the allowlist and its two enforcement points, and the `Provisioned` ledger, with chainsaw suites proving delivery, gating, pruning, allowlist refusal with the webhook disabled, and project isolation against separately routed control planes.
...
1. **Should the allowlist stay static configuration, or become a platform-admin-only API object?** Static is more auditable; an API object is more operable. Moving it later does not change the two enforcement points.
...
6. **First-party providers only, or third-party?** Under third-party providers the allowlist becomes load-bearing rather than defense-in-depth, and per-service identity becomes a requirement rather than an improvement.
