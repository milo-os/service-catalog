...
2. In-memory maps built from watch replay are **not restart-safe**: a `ServiceConsumer` deleted while the operator was down produces no replay event on restart, so a map-based design would never fire teardown for it. The informer’s initial list supplies the current objects on startup, and the provider teardown finalizer keeps deletion-visible objects present until cleanup completes. Listing on each event-driven reconcile therefore remains self-healing without a per-object periodic timer.
...
3. **Provider-side Service changes.** The provider resolves `ServiceConsumer.spec.serviceRef.name` through provider-project `Service` objects. If a service’s canonical name changes without a corresponding `ServiceConsumer` event, the provider does not currently receive a trigger. Service names should be treated as immutable; if that guarantee changes, add an explicit `Service` watch rather than restoring a per-consumer periodic timer.
