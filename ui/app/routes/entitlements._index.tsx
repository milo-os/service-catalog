...
export async function loader({ request }: LoaderFunctionArgs) {
  try {
    ...
    const uniqueServiceNames = [...new Set(entitlements.map((e) => e.spec.serviceRef.name))];
    ...
    return json({ entitlements, serviceDisplayNames } satisfies LoaderData);
  } catch (e) {
    return json({
      entitlements: [],
      serviceDisplayNames: {},
...
export default function EntitlementsIndex() {
  const { entitlements, serviceDisplayNames, error } = useLoaderData<typeof loader>() as LoaderData;
                  ...
                  return (
                    <TableRow key={e.metadata.name}>
                      <TableCell>
                        ...
                        >
                          {serviceDisplayNames[serviceRef] ?? serviceRef}
                        </Link>
                      </TableCell>
                      <TableCell>
                        {phase ? (
