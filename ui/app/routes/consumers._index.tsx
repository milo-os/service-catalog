...
import type { KubeList, Service, ServiceConsumer } from "~/lib/types";
...
export async function loader({ request }: LoaderFunctionArgs) {
  try {
    ...
    const uniqueServiceNames = [...new Set(consumers.map((c) => c.spec.serviceRef.name))];
    ...
    uniqueServiceNames.forEach((name, i) => {
      const result = serviceResults[i];
      if (result.status === "fulfilled") {
        serviceDisplayNames[name] = result.value.spec.displayName;
      }
    });

    return json({ consumers, serviceDisplayNames } satisfies LoaderData);
  } catch (e) {
    return json({
      consumers: [],
      serviceDisplayNames: {},
...
export default function ConsumersIndex() {
  const { consumers, serviceDisplayNames, error } = useLoaderData<typeof loader>() as LoaderData;
                    ...
                    return (
                      <TableRow key={consumer.metadata.name}>
                        ...
                        <TableCell>
                          ...
                          >
                            {serviceDisplayNames[consumer.spec.serviceRef.name] ?? consumer.spec.serviceRef.name}
                          </Link>
                        </TableCell>
                        <TableCell>
